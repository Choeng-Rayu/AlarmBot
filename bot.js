...
//   const webhookYU= 'https://alarmbot-mbkv.onrender.com';
//   setTimeout(async () => {
//     try {
//       const webhookUrl = `${webhookYU}/webhook`;
...
// Enhanced user schema with unique alarm IDs
...
// Store active jobs and timeouts
const activeJobs = {};
...
// Improved scheduling with transaction handling
async function scheduleUserAlarms(chatId) {
  ...
  try {
    ...
    for (const alarm of user.alarms) {
      const [hour, minute] = alarm.time.split(':').map(Number);
      const jobName = `alarm_${chatId}_${alarm._id}`;
      
      try {
        activeJobs[jobName] = schedule.scheduleJob(
          { hour, minute, tz: 'Asia/Phnom_Penh' },
          async () => {
            await sendAlarmMessage(chatId, alarm._id);
          }
        );
        alarm.jobName = jobName;
      } catch (err) {
    ...
    console.error('Scheduling transaction error:', err);
  } finally {
    session.endSession();
  }
}

// Enhanced alarm message with proper timeout handling
async function sendAlarmMessage(chatId, alarmId) {
  const user = await User.findOne({ chatId });
  if (!user) return;
  ...
  const options = {
    reply_markup: {
      inline_keyboard: [
        [{ text: "I did it!", callback_data: `ack_${alarmId}` }],
        [{ text: "Skip", callback_data: `skip_${alarmId}` }]
      ]
    }
  };
  ...
  try {
    ...
    // Handle timeout with clear cleanup
    const timeout = setTimeout(async () => {
      const updatedUser = await User.findOne({ chatId });
      if (!updatedUser) return;

      const updatedAlarm = updatedUser.alarms.id(alarmId);
      if (updatedAlarm?.pending) {
        await bot.sendMessage(chatId, `⚠️ You missed your alarm at ${updatedAlarm.time}. Streak reset!`);
        updatedUser.streak = 0;
        updatedAlarm.pending = false;
      ...
      activeTimeouts.delete(alarmId.toString());
    }, 60 * 60 * 1000);

    activeTimeouts.set(alarmId.toString(), timeout);
  } catch (err) {
    console.error('Alarm message error:', err);
  }
}
...
async function initializeServer() {
  try {
    ...
    await Promise.all(users.map(user => scheduleUserAlarms(user.chatId)));
    
    // Check pending alarms
    const pendingUsers = await User.find({ 'alarms.pending': true });
      ...
      for (const alarm of user.alarms) {
        if (alarm.pending && alarm.sentAt) {
          ...
          if (elapsed > 60 * 60 * 1000) {
            ...
            const timeout = setTimeout(async () => {
              //... similar to original timeout handler
            }, remaining);
            activeTimeouts.set(alarm._id.toString(), timeout);
          }
        }
      }
    }
...
app.listen(PORT, async () => {
  console.log(`Server running on port ${PORT}`);
  try {
    await bot.setWebHook(`https://alarmbot-mbkv.onrender.com`);
    console.log('Webhook set successfully');
...
bot.onText(/\/start/, async (msg) => {
  ...
  try {
    ...
    await bot.sendMessage(chatId, 
      `Welcome! You can set up to 10 alarms.\n` +
      `Current alarms: ${user.alarms.length}/10`, 
      options
    );
  } catch (err) {
...
// Enhanced add alarm handler
bot.onText(/\/addalarm (.+)/, async (msg, match) => {
  const chatId = msg.chat.id;
  ...
  try {
    ...
    if (user.alarms.length >= 10) {
      return bot.sendMessage(chatId, "❌ Maximum 10 alarms allowed!");
    }

    if (user.alarms.some(a => a.time === time)) {
      return bot.sendMessage(chatId, "❌ This alarm already exists!");
    }
    ...
    const remaining = 10 - user.alarms.length;
    await bot.sendMessage(chatId, 
      `⏰ Alarm set for ${time}. ${remaining} slots remaining.`
    );
  } catch (err) {
    console.error('Add alarm error:', err);
    await bot.sendMessage(chatId, "❌ Failed to set alarm. Please try again.");
  }
});
...
bot.on('callback_query', async (callbackQuery) => {
  const chatId = callbackQuery.message.chat.id;
  const [action, alarmId] = callbackQuery.data.split('_');
  ...
  try {
    ...
    if (action === 'ack') {
      ...
      await bot.answerCallbackQuery(callbackQuery.id, { 
        text: `Great job! Streak: ${user.streak} days` 
      });
    } else if (action === 'skip') {
      alarm.pending = false;
      await user.save();
      await bot.answerCallbackQuery(callbackQuery.id, { text: "Skipped for today" });
    }

    // Cleanup timeout
    const timeout = activeTimeouts.get(alarmId);
    if (timeout) {
...
// ... other handlers (list alarms, stats, unsubscribe) with similar improvements ...

process.on('SIGINT', async () => {
  console.log('Shutting down gracefully...');
  // Cleanup jobs and timeouts
  Object.values(activeJobs).forEach(job => job.cancel());
...
//         );
//         alarm.jobName = jobName;
//         console.log(`Scheduled alarm ${jobName} for ${alarm.time}`);
//       } catch (err) {
//         console.error(`Failed to schedule job ${jobName}:`, err);
//       }
//     });
...
//     await user.save();
//   } catch (err) {
//     console.error(`Error scheduling alarms for chatId ${chatId}:`, err);
//   }
// }

// // Enhanced alarm message sending
// async function sendAlarmMessage(chatId, alarmIndex) {
//   try {
//     const user = await User.findOne({ chatId });
//     if (!user || alarmIndex >= user.alarms.length) return;
...
//     // Set timeout for alarm expiration
//     const timeoutDuration = 60 * 60 * 1000; // 1 hour
//     setTimeout(async () => {
//       await checkAlarmExpiration(chatId, alarmIndex);
//     }, timeoutDuration);

//   } catch (err) {
//     console.error(`Error sending alarm message to chatId ${chatId}:`, err);
//   }
// }

// // Check alarm expiration
// async function checkAlarmExpiration(chatId, alarmIndex) {
//   try {
//     const user = await User.findOne({ chatId });
//     if (!user || alarmIndex >= user.alarms.length) return;
...
//     const alarm = user.alarms[alarmIndex];
//     if (alarm.pending) {
//       await bot.sendMessage(chatId, `⚠️ You missed your alarm at ${alarm.time}. Streak reset!`);
//       user.streak = 0;
//       alarm.pending = false;
//       await user.save();
//     }
//   } catch (err) {
//     console.error(`Error checking alarm expiration for chatId ${chatId}:`, err);
//   }
// }
...
//             setTimeout(() => checkAlarmExpiration(user.chatId, i), remainingTime);
//           }
//         }
//       }
//       await user.save();
...
// // Command handlers (continued in next message due to length)...